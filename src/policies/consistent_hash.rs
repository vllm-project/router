...
impl ConsistentHashPolicy {
    ...
    fn extract_hash_key(&self, request_text: Option<&str>) -> String {
        let text = request_text.unwrap_or("");

        // 1. Try to extract session_params.session_id first (highest priority)
        ...
        // 5. Final fallback to using the entire request text (or a hash of it if too long)
        if text.len() > 100 {
...
impl LoadBalancingPolicy for ConsistentHashPolicy {
    ...
    fn needs_request_text(&self) -> bool {
        true // We need request text to extract session_id/user_id
    }
