        ...
        if let Some(prefill_choices) = prefill_json.get("choices").and_then(|v| v.as_array()) {
            ...
            for (decode_choice, prefill_choice) in choices.iter_mut().zip(prefill_choices.iter()) {
                if let (Some(decode_obj), Some(prefill_obj)) = (
                    ...
                    if let (Some(prefill_logprobs), Some(decode_logprobs)) = (
                        prefill_obj.get("logprobs"),
                        decode_obj.get_mut("logprobs"),
                    ) {
                        if let (Some(prefill_logprobs_obj), Some(decode_logprobs_obj)) = (
                            ...
                            if let (Some(prefill_token_logprobs), Some(decode_token_logprobs)) = (
                                prefill_logprobs_obj.get("token_logprobs").and_then(|v| v.as_array()),
                                decode_logprobs_obj.get("token_logprobs").and_then(|v| v.as_array()),
                            ) {
                                // Extract only prompt logprobs from prefill (exclude the 1 output token)
                                let prefill_prompt_only = &prefill_token_logprobs[..num_prompt_tokens.min(prefill_token_logprobs.len())];
                            ...
                            if let (Some(prefill_tokens), Some(decode_tokens)) = (
                                prefill_logprobs_obj.get("tokens").and_then(|v| v.as_array()),
                                decode_logprobs_obj.get("tokens").and_then(|v| v.as_array()),
                            ) {
                                // Extract only prompt tokens from prefill (exclude the 1 output token)
                                let prefill_prompt_tokens_only = &prefill_tokens[..num_prompt_tokens.min(prefill_tokens.len())];
                            ...
                            if let (Some(prefill_offsets), Some(decode_offsets)) = (
                                prefill_logprobs_obj.get("text_offset").and_then(|v| v.as_array()),
                                decode_logprobs_obj.get("text_offset").and_then(|v| v.as_array()),
                            ) {
                                // Extract only prompt offsets from prefill (exclude the 1 output token)
                                let prefill_prompt_offsets_only = &prefill_offsets[..num_prompt_tokens.min(prefill_offsets.len())];
                                ...
                                if !prefill_prompt_offsets_only.is_empty() {
                                    ...
                                    let base_offset = if let Some(prefill_tokens_arr) = prefill_logprobs_obj
                                        .get("tokens")
                                        .and_then(|v| v.as_array())
                                    {
                                        if num_prompt_tokens > 0 && prefill_tokens_arr.len() >= num_prompt_tokens {
                                            let last_token = &prefill_tokens_arr[num_prompt_tokens - 1];
                                    ...
                                    let adjusted_decode_offsets: Vec<Value> = decode_offsets
                                        .iter()
                                        .filter_map(|v| {
                                            v.as_i64().map(|offset| Value::from(offset + base_offset))
                                        })
                                        .collect();
                            ...
                            if let (Some(prefill_top_logprobs), Some(decode_top_logprobs)) = (
                                prefill_logprobs_obj.get("top_logprobs").and_then(|v| v.as_array()),
                                decode_logprobs_obj.get("top_logprobs").and_then(|v| v.as_array()),
                            ) {
                                // Extract only prompt top_logprobs from prefill (exclude the 1 output token)
                                let prefill_prompt_top_only = &prefill_top_logprobs[..num_prompt_tokens.min(prefill_top_logprobs.len())];
