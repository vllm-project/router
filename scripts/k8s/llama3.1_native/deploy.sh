...
# Deploy Llama 3.1 8B with vLLM native routing using helmfile
# Usage: ./deploy.sh [namespace] [gateway-provider]
#
# Examples:
#   ./deploy.sh                                # Deploy to llm-d-llama31-native with default (istioBench)
#   ./deploy.sh llm-d-llama31-native istio     # Deploy to llm-d-llama31-native with istio
#   ./deploy.sh my-namespace kgateway          # Deploy to my-namespace with kgateway

set -e

NAMESPACE="${1:-llm-d-llama31-native}"
GATEWAY_PROVIDER="${2:-default}"

echo "===================================="
echo "Deploying Llama 3.1 8B Native"
echo "===================================="
