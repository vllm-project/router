...
- **Router**: vllm-router with DP-aware routing (--data-parallel-size 8)
...
Important: DP8 is configured via `--data-parallel-size 8` flag in vLLM, NOT by creating 8 separate pods. Each pod runs a single vLLM process that internally manages 8 data parallel workers across 8 GPUs.
