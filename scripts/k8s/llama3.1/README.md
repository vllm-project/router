...
- **Parallelism**: DP8TP1 (Data Parallel 8, Tensor Parallel 1) via `--data-parallel-size 8` flag
...
| Parallelism | TP8DP1 | DP8TP1 (via --data-parallel-size flag) |
| Tensor Parallel | 8 | 1 |
| Data Parallel | 1 | 8 |
...
**Important**: Data parallelism is configured via the `--data-parallel-size 8` flag in vLLM, NOT by creating 8 separate pods. Each pod (prefill and decode) runs a single vLLM server process that internally manages 8 data parallel workers across 8 GPUs.
