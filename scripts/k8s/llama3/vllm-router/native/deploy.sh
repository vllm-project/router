...
# Deploy vLLM with vllm-router (native load balancing)
# Usage: ./deploy.sh
...
echo "=========================================="
echo "Deploying vLLM with vllm-router (native)"
echo "=========================================="
...
echo ""
echo "Deploying vLLM backends with helmfile..."
cd "$(dirname "$0")"
...
echo ""
echo "Waiting for vLLM backends to be ready..."
kubectl wait --for=condition=ready --timeout=600s \
    pod -l llm-d.ai/role=decode -n "$NAMESPACE" 2>/dev/null || \
    echo "Backend pods not ready yet, continuing..."
